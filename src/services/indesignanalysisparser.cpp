...
InDesignAnalysisParser::Result InDesignAnalysisParser::parse(const QByteArray &json)
{
    Result result;
    ...
    {
        result.errorMessage = QStringLiteral("JSON inválido: %1").arg(parseError.errorString());
    ...
    {
        result.errorMessage = QStringLiteral("La respuesta de InDesign no contiene un objeto JSON.");
    ...
    {
        result.errorMessage =
            root.value(QStringLiteral("error")).toString(QStringLiteral("InDesign devolvió un error."));

        return result;
    }

    result.documentId = root.value(QStringLiteral("documentId")).toInteger();

    result.documentName = root.value(QStringLiteral("documentName")).toString();

    result.documentPath = root.value(QStringLiteral("documentPath")).toString();
...
LinkInfo InDesignAnalysisParser::parseLink(const QJsonObject &object)
{
    ...
    link.actualResolution = {actual.value("x").toDouble(), actual.value("y").toDouble()};

    const QJsonObject effective = object.value("effectiveResolution").toObject();

    link.effectiveResolution = {effective.value("x").toDouble(), effective.value("y").toDouble()};

    const QJsonObject scale = object.value("scale").toObject();

    link.scale = {scale.value("horizontal").toDouble(100.0), scale.value("vertical").toDouble(100.0)};

    link.rotation = object.value("rotation").toDouble();

    const QJsonObject flip = object.value("flip").toObject();

    link.flip = {flip.value("horizontal").toBool(false), flip.value("vertical").toBool(false)};
