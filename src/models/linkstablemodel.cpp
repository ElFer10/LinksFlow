...
int LinksTableModel::rowCount(const QModelIndex &parent) const {
  if (parent.isValid()) {
    return 0;
  }
...
int LinksTableModel::columnCount(const QModelIndex &parent) const {
  if (parent.isValid()) {
    return 0;
  }
...
QVariant LinksTableModel::data(const QModelIndex &index, int role) const {
  ...
  const bool hasUnavailableMetadata = link.state == LinkProcessState::Missing ||
  ...
  case EffectiveResolutionColumn:
    ...
    return formatResolution(link.effectiveResolution);
...
bool LinksTableModel::setData(const QModelIndex &index, const QVariant &value,
                              int role) {
  if (!index.isValid()) {
    return false;
  }
...
QVariant LinksTableModel::headerData(int section, Qt::Orientation orientation,
                                     int role) const {
...
Qt::ItemFlags LinksTableModel::flags(const QModelIndex &index) const {
  if (!index.isValid()) {
...
void LinksTableModel::setLinks(const QList<LinkInfo> &links) {
  beginResetModel();
...
QList<LinkInfo> LinksTableModel::links() const { return m_links; }

LinkInfo LinksTableModel::linkAt(int row) const {
...
QString LinksTableModel::formatFileSize(qint64 bytes) const {
  if (bytes < 1024) {
    return tr("%1 B").arg(bytes);
  }

  const double kilobytes = bytes / 1024.0;
  ...
  if (megabytes < 1024.0) {
    return tr("%1 MB").arg(megabytes, 0, 'f', 1);
  }

  const double gigabytes = megabytes / 1024.0;

  return tr("%1 GB").arg(gigabytes, 0, 'f', 2);
}

QString
LinksTableModel::formatResolution(const Resolution2D &resolution) const {
...
bool LinksTableModel::resolutionsAreEqual(
    const Resolution2D &resolution) const {
  constexpr double tolerance = 0.5;

  return qAbs(resolution.x - resolution.y) < tolerance;
}
...
int LinksTableModel::selectedForProcessingCount() const {
  ...
  for (const LinkInfo &link : m_links) {
    if (link.state == LinkProcessState::Ready && link.process) {
      ++count;
    }
  }
