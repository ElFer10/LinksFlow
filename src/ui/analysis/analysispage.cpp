...
AnalysisPage::AnalysisPage(QWidget *parent) : QWidget(parent), m_previewService(new PreviewService(this))
{
    setupUi();
    setupConnections();
    clearInspector();
...
void AnalysisPage::setupUi()
{
    ...
    auto *description = new QLabel(tr("Seleccione los enlaces que desea procesar."), this);
    ...
    m_table->setEditTriggers(QAbstractItemView::SelectedClicked | QAbstractItemView::DoubleClicked);
    ...
    header->setSectionResizeMode(LinksTableModel::ProcessColumn, QHeaderView::ResizeToContents);

    header->setSectionResizeMode(LinksTableModel::PageColumn, QHeaderView::ResizeToContents);

    header->setSectionResizeMode(LinksTableModel::NameColumn, QHeaderView::Stretch);

    header->setSectionResizeMode(LinksTableModel::TypeColumn, QHeaderView::ResizeToContents);

    header->setSectionResizeMode(LinksTableModel::ColorColumn, QHeaderView::ResizeToContents);

    header->setSectionResizeMode(LinksTableModel::SizeColumn, QHeaderView::ResizeToContents);

    header->setSectionResizeMode(LinksTableModel::EffectiveResolutionColumn, QHeaderView::ResizeToContents);

    header->setSectionResizeMode(LinksTableModel::IccProfileColumn, QHeaderView::ResizeToContents);
...
{
    connect(m_exitButton, &QPushButton::clicked, this, &AnalysisPage::exitRequested);

    connect(m_backButton, &QPushButton::clicked, this, &AnalysisPage::backRequested);

    connect(m_processButton, &QPushButton::clicked, this, &AnalysisPage::processRequested);
    ...
    connect(m_model, &LinksTableModel::processSelectionChanged, this, &AnalysisPage::updateProcessingState);
    ...
    connect(m_previewService, &PreviewService::previewFailed, this, [this](int requestId, const QString &) {
        if (requestId != m_previewRequestId)
        {
            return;
        }
...
void AnalysisPage::updateInspector(const QModelIndex &proxyIndex)
{
    ...
    m_effectiveResolutionValue->setText(formatResolution(link.effectiveResolution));
    ...
    m_typeValue->setText(link.fileType.isEmpty() ? QStringLiteral("—") : link.fileType);

    m_colorModeValue->setText(link.colorMode.isEmpty() ? QStringLiteral("—") : link.colorMode);

    m_iccProfileValue->setText(link.iccProfile.isEmpty() ? QStringLiteral("—") : link.iccProfile);

    if (link.state == LinkProcessState::Missing || link.state == LinkProcessState::Error)
    {
...
QString AnalysisPage::formatScale(const Scale2D &scale) const
{
    ...
    return tr("%1 % × %2 %").arg(scale.horizontal, 0, 'f', 2).arg(scale.vertical, 0, 'f', 2);
}
...
void AnalysisPage::updateProcessingState()
{
    ...
    m_selectionSummary->setText(tr("%1 de %2 enlaces seleccionados").arg(selected).arg(processable));
...
QList<LinkInfo> AnalysisPage::links() const
{
    return m_model->links();
}
